...
function formatQueryString(params) {
    ...
    const queryParts = Object.keys(params)  //generate an array whose items are strings representing each key-value pair
        .map(key => `${encodeURIComponent(key)}=${encodeURIComponent(params[key])}`);
    /*For each key, it uses the built-in JavaScript function
     encodeURIComponent, which converts strings to URL safe formats
     by escaping characters like spaces to %20. It does the same for
     each value, connecting the key and value with an = character.
     */
    return queryParts.join('&'); // returns a single string generated by joining each array item with the "&" character.
}
